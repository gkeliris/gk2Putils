function theAxis = msubplot(nrows, ncols, thisPlot,figHandle)
%MSUBPLOT - create axes in tiled positions.
%   H = MSUBPLOT(m,n,p), or MSUBPLOT(mnp), breaks the Figure window
...
% 
%       MSUBPLOT(2,1,1), PLOT(income)
%       MSUBPLOT(2,1,2), PLOT(outgo)
% 
%   plots income on the top half of the window and outgo on the
%   bottom half.
% 
%   MSUBPLOT(m,n,p), if the axis already exists, makes it current.
%   MSUBPLOT(m,n,P), where P is a vector, specifies an axes position
%   that covers all the msubplot positions listed in P.
%   MSUBPLOT(H), where H is an axis handle, is another way of making
%   an axis current for subsequent plotting commands.
%
%   MSUBPLOT('position',[left bottom width height]) creates an
%   axis at the specified position in normalized coordinates (in 
%   in the range from 0.0 to 1.0).
%
%   If a MSUBPLOT specification causes a new axis to overlap an
%   existing axis, the existing axis is deleted.  For example,
%   the statement MSUBPLOT(1,2,1) deletes all existing axes overlapping
%   the left side of the Figure window and creates a new axis on that
%   side.
%
%   MSUBPLOT(111) is an exception to the rules above, and is not
%   identical in behavior to MSUBPLOT(1,1,1).  For reasons of backwards
%   compatibility, it is a special case of msubplot which does not
...
delay_destroy = 0;
tol = sqrt(eps);
if narg == 0 % make compatible with 3.5, i.e. msubplot == msubplot(111)
    nrows = 111;
    narg = 1;
end
...
if narg == 1
    ...
    % all other numbers will be converted to mnp format:
        ...
        if nrows*ncols < thisPlot
            error('Index exceeds number of msubplots.');
        end
	kill_siblings = 1;
	if(code == 111)
	  create_axis   = 0;
...
elseif narg == 3
    % passed in msubplot(m,n,p) -- we should kill overlaps
    % here too:
    kill_siblings = 1;
elseif narg == 4,
    kill_siblings = 1;
...
elseif(isempty(position))
    if (min(thisPlot) < 1)
        ...
        % This is the percent offset from the msubplot grid of the plotbox.
        PERC_OFFSET_L = 2*0.020;
        PERC_OFFSET_R = 2*0.020;
        PERC_OFFSET_B = PERC_OFFSET_L;
        PERC_OFFSET_T = PERC_OFFSET_R;
        if nrows > 2
            PERC_OFFSET_T = 0.96*PERC_OFFSET_T;
            PERC_OFFSET_B = 0.96*PERC_OFFSET_B;
        end
        if ncols > 2
