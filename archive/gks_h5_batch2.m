 if isfile(fullfile('D:\1.3 MeCP2_cohort1_22 weeks raw and processed','exp_description.mat'))
    load(fullfile('D:\1.3 MeCP2_cohort1_22 weeks raw and processed','exp_description'));
    fprintf('Loaded exp_description from disk\n');
    diary(fullfile('D:\1.3 MeCP2_cohort1_22 weeks raw and processed',[datestr(now,'yyyy-mm-dd_HHMMSS'),'_log.txt']))
else
     gks_mecp2_description_cohort1_22weeks;
end
    ...
    for mi=1:numel(m)
        ...
        for ei=1:numel(e)
            ...
            if ~isfile(fullfile(descr.(c{ci}).(m{mi}).(e{ei}).procPath,'stim.mat')) && ...
                ...
                if isempty(d)                    d=dir(fullfile(descr.(c{ci}).(m{mi}).(e{ei}).procPath,'*.h5'));
                    if isempty(d)
                ...
                if strcmp(answer,'q')
                    save(fullfile('D:\1.3 MeCP2_cohort1_22 weeks raw and processed','exp_description'),'descr');
                    diary off
                    return
                end
                if strcmp(answer,'s')
