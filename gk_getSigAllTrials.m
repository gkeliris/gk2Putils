...
if t_dur/stim.Times.frame_fs < 2
    t_ext_sec = 0.5; % extend the duration for 0.5 sec (for STIMON period)
    t_ext = ceil(t_ext_sec*stim.Times.frame_fs);
else
    t_ext = 0;
end
% Calculate the total trial duration
trial_dur=t_before_frames + t_dur + max(t_after_frames,t_ext);

sig.stimValues=stim.Values;
sig.t=[-t_before_frames:(t_dur + max(t_after_frames,t_ext) -1)]./stim.Times.frame_fs(z);
...
i_to = stim.Times.frame_onsets(z,1:Ntrials) + t_dur + max(t_after_frames,t_ext) -1;
