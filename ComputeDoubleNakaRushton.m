function [response] = ComputeDoubleNakaRushton(params,contrast)
...
if (sigma < 0 || exponent < 0 || exponent1 < 0)
    ...
    expContrast_ = (contrast).^exponent_;
    expContrast1_ = contrast.^exponent1_;
    sigma1 = sigma.^exponent1;
    sigma1_ = sigma_.^exponent1_;
    response = A*(expContrast ./ (expContrast1 + sigma1))...
        - A_*(expContrast_ ./ (expContrast1_ + sigma1_));
end
